...
class SYSAction
{
    ...
    {
//        $DB = BaseWork::DB();
//        $query = "SELECT slm.left_menu_name, slm.left_menu_url, slm.left_menu_belong_id, slm.left_menu_id
//		FROM sys_left_menu AS slm left join sys_grp_menu AS sgm ON slm.left_menu_id = sgm.left_menu_id
//		WHERE sgm.group_id = :group_id AND slm.left_menu_on = 1 AND slm.left_menu_type = 1 ORDER BY slm.left_menu_sn asc, slm.left_menu_id asc";
//        $stat = $DB->prepare($query);
//        $stat->bindValue(':group_id', $_SESSION['SYS_UserGroup'], PDO::PARAM_INT);
//        $stat->execute();
        ...
        foreach ($results as $row) {
            ...
            if ($row['left_menu_belong_id'] == "" && $row['left_menu_url'] == "") {
                ...
                echo '
					...
					';
                //撈出第二層選單資料
//                $query2 = "SELECT slm.left_menu_name, slm.left_menu_url, slm.left_menu_belong_id, slm.left_menu_id
//				FROM sys_left_menu AS slm left join sys_grp_menu AS sgm ON slm.left_menu_id = sgm.left_menu_id
//				WHERE sgm.group_id = :group_id AND slm.left_menu_belong_id = :left_menu_belong_id AND slm.left_menu_on = 1 AND slm.left_menu_type = 1
//				ORDER BY slm.left_menu_sn asc, slm.left_menu_id asc";
//                $stat2 = $DB->prepare($query2);
//                $stat2->bindValue(':group_id', $_SESSION['SYS_UserGroup'], PDO::PARAM_INT);
//                $stat2->bindValue(':left_menu_belong_id', $row['left_menu_id'], PDO::PARAM_INT);
//                $stat2->execute();
...
//        $DB = BaseWork::DB();
//        $query = "SELECT slm.left_menu_name, slm.left_menu_url, slm.left_menu_belong_id, slm.left_menu_id
//		FROM sys_left_menu AS slm left join com_grp_menu AS cgm ON slm.left_menu_id = cgm.left_menu_id
//		WHERE cgm.temple_id = :temple_id AND slm.left_menu_on = 1 AND slm.left_menu_type = 2 ORDER BY slm.left_menu_sn asc, cgm.left_menu_id asc";
//        $stat = $DB->prepare($query);
//        $stat->bindValue(':temple_id', $_SESSION['COM_UserID'], PDO::PARAM_INT);
//        $stat->execute();
//        $results = $stat->fetchAll(PDO::FETCH_ASSOC);
//
//        echo '<ul class="nav nav-list">';
...
//                $query2 = "SELECT slm.left_menu_name, slm.left_menu_url, slm.left_menu_belong_id, slm.left_menu_id
//				FROM sys_left_menu AS slm left join com_grp_menu AS cgm ON slm.left_menu_id = cgm.left_menu_id
//				WHERE cgm.cgroup_id = :cgroup_id AND slm.left_menu_belong_id = :left_menu_belong_id AND slm.left_menu_on = 1 AND slm.left_menu_type = 2
//				ORDER BY slm.left_menu_sn asc, cgm.left_menu_id asc";
//                $stat2 = $DB->prepare($query2);
//                $stat2->bindValue(':cgroup_id', $_SESSION['COM_UserGroup'], PDO::PARAM_INT);
//                $stat2->bindValue(':left_menu_belong_id', $row['left_menu_id'], PDO::PARAM_INT);
//                $stat2->execute();
//                $results2 = $stat2->fetchAll(PDO::FETCH_ASSOC);
//                foreach ($results2 as $row2) {
...
//    }
    ...
    public static function Menu_MAP()
    {
        ...
        if ($Belong_Page_ID) {
            $Belong_Page_ID2 = self::SQL_Data('sys_left_menu', 'left_menu_id', $Belong_Page_ID, 'left_menu_belong_id');
            if ($Belong_Page_ID2) {
                $Belong_Page_Name2 = self::SQL_Data('sys_left_menu', 'left_menu_id', $Belong_Page_ID2, 'left_menu_name');
...
//            $DB = BaseWork::DB();
//            $query = "SELECT administrator_user, group_id, administrator_on FROM sys_administrator WHERE administrator_user = :administrator_user AND administrator_pwd = :administrator_pwd";
//            $stat = $DB->prepare($query);
//            $stat->bindValue(':administrator_user', $loginUsername);
//            $stat->bindValue(':administrator_pwd', hash('sha512', $password));
//            $stat->execute();
//            $row = $stat->fetch();
...
//            $DB = BaseWork::DB();
//            $query = "SELECT temple_id, temple_master_id, temple_account, temple_switch FROM com_temple WHERE temple_account = :temple_account AND temple_password = :temple_password";
//            $stat = $DB->prepare($query);
//            $stat->bindValue(':temple_account', $loginUsername);
//            $stat->bindValue(':temple_password', $password);
//            $stat->execute();
//            $row = $stat->fetch();
...
class ONE
{
    ...
    public static function pay($pay_type, $amount, $ScanCode, $shopOrder, $CommodityInfo, $vm_id)
    {
        ...
        //寫入登入紀錄
//        $DB = BaseWork::DB();
//        $insertSQL = "INSERT INTO one_log ( msg ) VALUES ( :msg )";
//        $stat2 = $DB->prepare($insertSQL);
//        $stat2->bindValue(':msg', $msg);
//        $stat2->execute();
...
class UIC
{
    ...
    public static function pay($pay_type, $amount, $ScanCode, $shopOrder, $CommodityInfo, $vm_id)
    {
        ...
        //寫入登入紀錄
//        $DB = BaseWork::DB();
//        $insertSQL = "INSERT INTO uic_log ( msg ) VALUES ( :msg )";
//        $stat2 = $DB->prepare($insertSQL);
//        $stat2->bindValue(':msg', $msg);
//        $stat2->execute();
